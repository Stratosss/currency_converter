...
API_KEY = '*Create account and retrieve your API*'    #API token from https://www.freecurrencyapi.com
...
def inputs():   #Ensures correct values are inserted with the option to quit on steps 1 and 2
    while True:
        a = input('Select the currency you want to exchance from (pres q for quit): ').upper() #accepts both capital and lowercase curr
        if a == "q":
            sys.exit()
        else:
            if a in CURRENCIES:
    ...
    while True:
        b = input('Select the currency you want to exchance to: ').upper()  #accepts both capital and lowercase curr
        if b == "q":
            sys.exit()
        else:
            if b in CURRENCIES:
